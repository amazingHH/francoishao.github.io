---
layout: home
title: Home
---

# Hi, I'm **Francois Hao**
<p align="center">
  <img src="Image (1).jpeg" width="180" style="border-radius: 50%;">
</p>
PhD Researcher at the Battery System Group & Connectivity and Navigation Systems Research Group, WMG, University of Warwick, UK.
I'm working on **Powerline Communication (PLC) in Smart Battery Systems**

---

## 🔬 Research Focus

I work on:

- Powerline communication (PLC) through battery cells
- Signal integrity under electrochemical variation
- State-of-Health (SOH) monitoring using communication signals
- Scalable battery network architectures

---

## 📌 Current Work

I am currently investigating how PLC performance varies across different battery chemistries and cycling conditions, and how this can be used for real-time diagnostics in large battery packs.

---

## 🧠 Skills & Tools

- MATLAB / Python
- NI LabVIEW
- VNA measurement & RF testing
- Embedded systems C/C++, Linux, RTOS, QNX
- Battery cycling analysis

---

## 📫 Contact

- Email: francois.hao@warwick.ac.uk  
- GitHub:https://github.com/amazingHH  
- LinkedIn: https://www.linkedin.com/in/fran%C3%A7ois-he-hao-5195b217b/
